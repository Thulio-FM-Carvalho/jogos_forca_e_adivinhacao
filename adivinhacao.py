import random


def jogar():
    print("#################################")
    print("Bem vindo no jogo de adivinhação!")
    print("#################################")

    numero_secreto = random.randrange(1, 101)
    total_de_tentativas = 0
    pontos = 1000

    print("Qual nível de dificuldade?")
    print("(1) Fácil (2) Médio (3) Difícil", numero_secreto)

    nivel = input("Digite o nível: ")

    if nivel == 1:
        total_de_tentativas = 20
    elif nivel == 2:
        total_de_tentativas = 10
    else:
        total_de_tentativas = 5

    for rodada in range(1, total_de_tentativas + 1):
        print("Tentativa {} de {}".format(rodada, total_de_tentativas))
        entrada_str = input("Digite o seu número entre 1 e 100: ")
        print("Você digitou ", entrada_str)
        entrada = int(entrada_str)

        if entrada < 1 or entrada > 100:
            print("Você deve digitar um número entre 1 e 100")
            continue

        acertou = entrada == numero_secreto
        maior = entrada > numero_secreto
        menor = entrada < numero_secreto

        if acertou:
            print("Você acertou e fez {} pontos!".format(pontos))
            break
        else:
            if maior:
                print("Você errou! O numero digitado foi maior do que o número secreto.")
            elif menor:
                print("Você errou! O numero digitado foi menor do que o numero secreto.")
            pontos_perdidos = abs(numero_secreto - entrada)
            pontos = pontos - pontos_perdidos

    print("Fim do jogo.")


if __name__ == "__main__":
    jogar()
